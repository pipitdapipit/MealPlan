...
document.addEventListener(`DOMContentLoaded`, async () => {
    ...
    setLoading(true);    
    const result = await fetch(`/get-meal-detail?recipeId=${recipeId}`);
    if(!result.ok) {
        setLoading(false);        
...
function setLoading(isLoading) {
    ...
    const loadingText = document.getElementById(`message-text`);
    if(isLoading) {        
        loadingText.style.display = "block";
        return containerIngredients.style.display = "none";
    } else {
        loadingText.style.display = "none";
        return containerIngredients.style.display = "flex";
    }
}

function showDetail(detail) {
    const textTitle = document.getElementById(`text-title`);
    const textReadyInMinutes = document.getElementById(`text-readyInMinutes`);
    const textPricePerServing = document.getElementById(`text-pricePerServing`);
    const textHealthScore = document.getElementById(`text-healthScore`);
    const textSummary = document.getElementById(`text-summary`);
    const imageRecipe = document.getElementById(`image-ingredients`);
    const titleDoc = document.getElementById(`title-doc`);
    
    imageRecipe.setAttribute(`src`, detail.image)
    textTitle.innerHTML = detail.title;
    textReadyInMinutes.innerHTML = detail.readyInMinutes;
    textPricePerServing.innerHTML = detail.pricePerServing;
    textHealthScore.innerHTML = detail.healthScore;
    textSummary.innerHTML = detail.summary;
    titleDoc.innerHTML = detail.title
}
function showIngredients(data) {
    const listView = document.getElementById(`lview-igrd`);

    data.extendedIngredients.forEach((igrd) => {
        const igrdWrapper = document.createElement(`igrd-wrapper`);
        igrdWrapper.innerHTML = `
             <div class="item-ingredients">
                        <div class="text-ii">
                            <h1>${igrd.name}</h1>
                            <div class="subtext-ii">
                                <p><span>${igrd.aisle}</span> |</p>
                                <p>Amount <span>${igrd.measures.metric.amount}${igrd.measures.metric.unitShort}</span></p>                                
                            </div>
                        </div>
                    </div>
        `;
        listView.appendChild(igrdWrapper);
    });
}
function showMessage(message) {
    ...
    loadingText.style.display = "block";
    return loadingText.innerHTML = message;
